...
class UnsafeMemory {
	...
	var realStartTimes = new long[nThreads];
        var realEndTimes = new long[nThreads];
	for (var i = 0; i < nThreads; i++){
            realStartTimes[i] = System.nanoTime();
	    t[i].start ();}
	for (var i = 0; i < nThreads; i++){
            realEndTimes[i] = System.nanoTime();
	    t[i].join ();}
	long realtime = 0;
        for (var i = 0; i < nThreads; i++){
            realtime += realEndTimes[i] - realStartTimes[i];
        }
	long cputime = 0;
	for (var i = 0; i < nThreads; i++)
